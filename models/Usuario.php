...
class Usuario extends ActiveRecord
{
    ...
    public function validarNuevaCuenta()
    {
        ...
        if(strlen($this->password) < 6) self::$alertas['error'][] = 'El password debe ser de al menos 6 caracteres'; 
    ...
    public function comprobarPassordAndVerificado($password)
    {
        $resultado = password_verify($password, $this->password);

        if(!$resultado || !$this->confirmado)
        {
            self::$alertas['error'][] = "Password incorrecto o cuenta no ha sido cerificada";
        }
        else
        {
            return true;
